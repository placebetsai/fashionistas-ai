...
export async function generateMetadata(): Promise<Metadata> {
  return {
    title: 'Deals & Sales | Fashionistas.ai',
...
export default function CategoryPage() {
  const filtered = articles.filter((a) => a.subcategory === 'Deals' || a.subcategory === 'Shopping');
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px 80px' }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <p style={{ color: '#666', fontSize: 16, maxWidth: 500, margin: '0 auto', lineHeight: 1.6, fontFamily: 'Inter, system-ui, sans-serif' }}>
            The best fashion and beauty deals, updated regularly.
          </p>
        </div>
        ...
        <div style={{
          ...
          {filtered.map((article) => (
            <a key={article.slug} href={`/blog/${article.slug}`} className="category-card" style={{
              ...
              <div className="article-img-deals" style={{
                width: '100%',
                height: 180,
                display: 'flex',
                alignItems: 'center',
